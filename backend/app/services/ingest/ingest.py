...
from app.services.shared.blob_storage import delete_pdf, upload_pdf
from app.services.shared.embeddings import embed_documents
from app.services.ingest.chunker import chunk_document
from app.services.ingest.google_drive import (
    DriveFile,
...
# ---------- Read-only queries (used by ingest router) ----------


async def list_documents(
    pool: asyncpg.Pool, *, status: str | None = None
) -> list[dict]:
    """List synced documents, optionally by status. Used by API layer."""
    async with pool.acquire() as conn:
        if status:
            rows = await conn.fetch(
                """
...
async def list_sync_runs(pool: asyncpg.Pool, *, limit: int = 20) -> list[dict]:
    """List recent sync runs. Used by API layer."""
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            """
...
async def get_sync_run(pool: asyncpg.Pool, run_id: str) -> dict | None:
    """Fetch a sync run and its items. Returns None if not found. Used by API layer."""
    async with pool.acquire() as conn:
