...
export default function Sidebar({
  ...
  return (
    ...
    >
      ...
      <div className={`border-b px-2 pt-1 pb-3 ${s.divider}`}>
        <Link
          href="/"
          className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition-colors ${s.item}`}
        >
      ...
      <nav className="flex-1 overflow-y-auto px-2 pt-3 pb-3">
        <p className={`mb-2 px-3 text-xs font-medium ${s.muted}`}>Recent chats</p>
        {conversations.length > 0 ?
          groupByDate(conversations).map((group) => (
            <div key={group.label} className="mb-3">
              <p className={`px-3 py-1.5 text-xs font-medium ${s.muted}`}>
                {group.label}
              </p>
              {group.items.map((c) => (
                <button
